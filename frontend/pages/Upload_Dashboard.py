...
def render_page() -> None:
    ...
    logger.info(
        f"Loaded { len(runs)} ETL run(s) for Upload Dashboard (page={page} limit={limit})",
    )

    selected_run = _render_runs_table(runs, limit, offset, total_entries)

    if selected_run is None:
        logger.debug("No specific DAG run selected on dashboard table")
        _render_table_controls(limit, page, total_entries)
        return

    logger.info(
        f"Rendering run details for dag_run_id={selected_run.get('dag_run_id')}"
    )
    _render_table_controls(limit, page, total_entries)
...
def _render_runs_table(
    ...
    if not runs:
        logger.info("No ETL runs returned for upload dashboard table")
    ...
    result = _find_run_by_number(runs, final_run_number)
    if result:
        logger.debug(
            f"Row {result.get('_row_number')} selected for dag_run_id={result.get('dag_run_id')}"
        )
    else:
        logger.debug("No row selected on current dashboard table page")
    return result
...
def _render_summary(summary: dict[str, Any]) -> None:
    ...
    logger.debug(
        ...
        len(summary.get("duplicate_sources", [])),
        len(summary.get("failed_sources", [])),
    )
...
def _render_table_controls(limit: int, page: int, total_entries: Any) -> None:
    ...
    if (
        ...
        logger.info(
            f"Updating dashboard pagination to page={target_page} limit={new_limit}"
        )
...
def _get_cached_summary(dag_run_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning(
            f"Unable to fetch cached summary for dag_run_id={dag_run_id}: {exc}",
        )
        return None
